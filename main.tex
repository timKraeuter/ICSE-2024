...
\textbf{(1)} We describe the improvements and new features added to the Visual Debugger since our last publication \cite{krauterVisualDebuggerTool2022}, see \autoref{sec:visualDebugger}.
...
Traditionally, stack frame variables are represented textually, such as in \autoref{fig:variablesIntellij}, a screenshot of the variables view in \intellij{} for a Binary Search Tree (BST).

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.2\textwidth]{images/variables.png}
    \caption{BST in the variables view of \intellij}
...
However, the coloring in \autoref{fig:variablesVisualDebugger} highlights variable changes and additions, a new feature of the Visual Debugger discussed in \autoref{subsec:improvements}.

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.25\textwidth]{images/variables-visual-debugger.png}
    \caption{BST as an object diagram using the Visual Debugger}
...
The Visual Debugger automatically updates the debug information just as the \intellij{} debugger whenever a user steps through the source code or reaches a new breakpoint.
